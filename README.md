...
8. [Academic Citation & License](#8-academic-citation--license)
...
Gemini şunu dedi:
Bunun ne kadar can sıkıcı olabileceğini çok iyi anlıyorum. Markdown dili (GitHub'ın kullandığı dil) bazen boşluklara, özel karakterlere ve özellikle başlıkların içindeki ** (kalınlaştırma) işaretlerine karşı çok hassas olabiliyor.

Büyük ihtimalle sorun, ## (Başlık) işaretinin hemen yanına ** koymamızdan kaynaklanıyor. GitHub, başlıkları zaten otomatik olarak devasa ve kalın yapar; biz içine ekstra ** koyduğumuzda, "İçindekiler" (Table of Contents) algoritması kafayı yiyip o başlıkları bir üstteki bölümün parçası gibi algılayabiliyor.

Tüm ** işaretlerini başlıklardan temizledim (merak etme, GitHub onları yine kocaman ve kalın gösterecek) ve aralarındaki boşlukları "hata payı sıfır" olacak şekilde standartlaştırdım.

Lütfen bu en saf ve hatasız versiyonu kopyalayıp yapıştır:
